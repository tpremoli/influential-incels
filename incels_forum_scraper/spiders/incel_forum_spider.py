...
class ForumSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        threads = response.css('div.structItem--thread a[data-tp-primary="on"]::attr(href)').getall()
    ...
    def parse_thread(self, response):
        ...
        is_first_page = not any(part.startswith('page-') for part in response.url.split('/'))
        if is_first_page:
            page_number = 1
        else:
        ...
        if is_first_page:
            ...
            if first_post:
                ...
                thread_item['author'] = first_post.css('.message-userDetails .message-name .username span::text').get()               
                thread_item['post_date'] = first_post.css('time.u-dt::attr(datetime)').get()
                thread_item['original_post_content'] = ' '.join(first_post.css('.message-content ::text').getall()).strip()
                
                #         "post_id": "post-13448716",
                thread_item['post_id'] = int(first_post.xpath('@data-content').get()[5:])
            else:
                # Fallback or default values if the first post isn't found
                thread_item['author'] = 'Unknown'
        ...
        for comment in comments:
            ...
            comment_item['post_id'] = int(comment.xpath('@data-content').get()[5:])
            
            # Removing quoted content before extracting text
            comment_html = comment.get()
            cleaned_html, reply_to_post_ids = extract_and_remove_quotes(comment_html)
            ...
            comment_item['comment_text'] = ' '.join(comment_selector.css('.message-content ::text').getall()).strip()
            comment_item['reply_to_post_id'] = reply_to_post_ids[0] if reply_to_post_ids else None

            # post metadata
            comment_item['commenter_username'] = comment.css('.message-userDetails .message-name .username span::text').get()            
            comment_item['comment_timestamp'] = comment.css('time.u-dt::attr(datetime)').get()
